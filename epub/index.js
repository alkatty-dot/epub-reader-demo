(function () {
  ...
  // ===== Layout helpers =====
  function setupTool() {
    // #toolButton 已改由 CSS 固定右上角，這裡只對齊 #viewer_event / #toolDiv / #toc
  ...
  function changeLayout() {
    ...
    start("viewer", "book2/item/standard.opf"); // 你原本的切換流程
  }
  ...
  function setupEventListeners() {
    ...
    document.addEventListener("keydown", function (e) {
      ...
      if (e.key === "ArrowRight") {
        if (pageProgressionDirection === "rtl") goToPreviousPage();
        else goToNextPage();
        e.preventDefault();
      } else if (e.key === "ArrowLeft") {
        if (pageProgressionDirection === "rtl") goToNextPage();
